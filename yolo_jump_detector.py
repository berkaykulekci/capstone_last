"""
LESS Analiz Sistemi - YOLO Atlayış Tespit Modülü
jump_detector.py'nin YOLO COCO-17 keypoint formatı için uyarlanmış versiyonu.

MediaPipe sürümünden tek fark: sinyal hesabında heel/foot_index yok.
contact_y = min(left_ankle_y, right_ankle_y)  [2 nokta, 6 yerine]
Tüm FSM, IC, MKF ve confidence mantığı aynıdır.
"""
import numpy as np
...
def _compute_signals(poses, test_side):
    """
    YOLO sürümü: contact_y yalnızca ankle noktalarından hesaplanır.
    MediaPipe'ta heel + foot_index de kullanılıyordu (6 nokta → 2 nokta).
    Sinyal gürültüsü hafif artabilir; Savitzky-Golay bunu dengeler.
    """
    n = len(poses)
    ...
    # contact_y: iki ayak bileğinin minimum Y'si → yere en yakın nokta
    # MediaPipe'ta 6 nokta (ankles, heels, toes) kullanılırdı.
...
def _generate_debug_plots(signals, jumps, label):
    ...
    fig.suptitle(f'YOLO Drop-Jump Detection — {label}', fontsize=14, fontweight='bold')
    ...
    ax = axes[0]
    ax.plot(frames, signals['contact_y'],     'b-', lw=0.8, label='contact_y [ankle only]')
    ...
    ax.set_title('Ayak Temas Y [YOLO: ankle only] + Hysteresis Eşikleri')
    ...
    plt.tight_layout()
    out_path = os.path.join(OUTPUT_DIR, f'debug_yolo_{label.lower()}.png')
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  [{label}] YOLO debug plot: {out_path}")
...
def detect_jumps(poses, test_side='right', label='', debug=True):
    ...
    Args:
        poses: np.ndarray (N_frames, 17, 3)  ← YOLO formatı
    ...
    if len(jumps_box) >= EXPECTED_JUMPS:
        ...
        print(f"  [{label}][YOLO] Kutu-referans → {len(result)} iniş (IC: {', '.join(ics)}) ✓")
        if debug:
    ...
    if len(jumps_fsm) >= EXPECTED_JUMPS:
        ...
        print(f"  [{label}][YOLO] FSM → {len(result)} atlayış (conf: {', '.join(confs)}) ✓")
        if debug:
            _generate_debug_plots(signals, result, label)
        return result
    ...
    if len(jumps_peak) >= EXPECTED_JUMPS:
        ...
        confs  = [f"{j.get('confidence', 0):.2f}" for j in result]
        print(f"  [{label}][YOLO] Peak-first → {len(result)} atlayış (conf: {', '.join(confs)}) ✓")
        if debug:
            _generate_debug_plots(signals, result, label)
        return result
    ...
    print(f"  [{label}][YOLO] Birleştirme → {len(merged)} atlayış ({counts})")
    if debug:
