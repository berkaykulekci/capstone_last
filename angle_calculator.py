"""
LESS Analiz Sistemi - Açı Hesaplama Modülü
...
def frontal_knee_valgus_check(poses, frame, side):
    ...
    """
    s = _side(side)
    hip_center_x = (poses[frame, LEFT_HIP, 0] + poses[frame, RIGHT_HIP, 0]) / 2.0
    knee_x = poses[frame, s['knee'], 0]
    foot_mid_x = (poses[frame, s['heel'], 0] + poses[frame, s['foot'], 0]) / 2.0

    # Diz, kalça merkezine göre ayağın iç tarafında mı?
    if side == 'left':
...
def foot_rotation_change(poses, frame_ic, frame_mkf, side):
    ...
    """
    s = _side(side)
    # IC'deki ayak vektörü (topuk→ayak ucu, x-z düzlemi)
    vec_ic = poses[frame_ic, s['foot'], [0, 2]] - poses[frame_ic, s['heel'], [0, 2]]
    # MKF'deki ayak vektörü
    vec_mkf = poses[frame_mkf, s['foot'], [0, 2]] - poses[frame_mkf, s['heel'], [0, 2]]
...
def foot_contact_symmetry(poses, frame, video_width):
    ...
    Y farkı threshold'dan büyükse asimetrik.
    Returns: y_farki (piksel).
    """
