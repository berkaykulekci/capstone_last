"""
LESS Analiz Sistemi - RTMPose Görsel Video Üretici
Pre-computed (N, RTM_NUM_KEYPOINTS, 3) poses kullanır — model yeniden çalıştırılmaz.

YOLO görselleştiriciden farkı: ayak (heel + toe) keypoint'leri çizilir ve
maddeler exact olduğundan "~ proxy" etiketleri yoktur.
"""
import cv2
import numpy as np
...
SKELETON_COLORED = [
    ...
    # Sol ayak — heel + toe (RTMPose'a özgü)
    (LEFT_ANKLE, LEFT_HEEL,       (255, 230, 120)),
    (LEFT_HEEL,  LEFT_FOOT_INDEX, (255, 230, 120)),
    (LEFT_ANKLE, LEFT_FOOT_INDEX, (255, 230, 120)),
    # Sağ ayak — heel + toe
    (RIGHT_ANKLE, RIGHT_HEEL,       (120, 230, 255)),
    (RIGHT_HEEL,  RIGHT_FOOT_INDEX, (120, 230, 255)),
    (RIGHT_ANKLE, RIGHT_FOOT_INDEX, (120, 230, 255)),
]
...
def _draw_skeleton(frame, kps, w, h):
    """Renk kodlu WholeBody (gövde + ayak) skeleton."""
...
def _draw_score_panel(frame, jump_results, active_keys, jump_idx, total_jumps, w, h):
    ...
    ph    = rows * lh + 40   # panel yüksekliği
    pw    = 210
    x1, y1 = pad, h - ph - pad
    x2, y2 = x1 + pw, h - pad
    ...
    ty = y1 + 16
    cv2.putText(frame, f'RTMPose | Atlayis {jump_idx+1}/{total_jumps}',
...
def create_rtm_camera_video(video_path: str, poses: np.ndarray, jumps: list,
                            results: list, kamera_tipi: str, output_path: str):
    """
    Args:
        video_path:   Kaynak video dosyası
        poses:        (N, K, 3) — normalize koordinatlar (Y terslenmiş, 0=alt)
    ...
    if not cap.isOpened():
        print(f"  [RTM-VIS] HATA: Video açılamadı: {video_path}")
        return
    ...
    while cap.isOpened():
        ...
        # Üst etiket
        _blend_rect(frame, 0, 0, 280, 30, alpha=0.55)
        cv2.putText(frame, f'RTMPose | {kamera_tipi.upper()} KAMERA',
    ...
    cap.release()
    out.release()
    print(f"  [RTM-VIS] Kaydedildi: {output_path}")
