"""
LESS Analiz Sistemi - YOLO için LESS Kural Modülü
...
def evaluate_m4(poses, ic, side):
    """M4: Plantar fleksiyon proxy — tibia dikeyden küçük açıysa toe-first tahmini."""
...
def evaluate_m5(poses, ic, side):
    ...
    return _result(5, valgus, valgus, puan,
                   f"Diz valgus {'VAR' if valgus else 'YOK'} (ankle proxy)", 'on')
...
def evaluate_m9(poses, ic, mkf, side):
    """M9: İç rotasyon proxy — IC→MKF knee-ankle x-offset negatif yönde değiştiyse."""
...
def evaluate_m10(poses, ic, mkf, side):
    """M10: Dış rotasyon proxy — IC→MKF knee-ankle x-offset pozitif yönde değiştiyse."""
    delta = foot_rotation_proxy(poses, ic, mkf, side)
    hata  = delta > M9_M10_OFFSET_THRESHOLD
...
def evaluate_m15(poses, mkf, side):
    ...
    return _result(15, valgus, valgus, puan,
                   f"MKF valgus {'VAR' if valgus else 'YOK'} (ankle proxy)", 'on')
...
def compute_total_score(combined):
    """Tüm 17 madde dahil toplam skor (max 19). M4/M9/M10 proxy ile yaklaşık."""
    ...
    return toplam, skorlar, sorted(YOLO_YAKLASIK_MADDE)
